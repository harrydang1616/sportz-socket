...
import {
    createMatchSchema, listMatchesQuerySchema, matchIdParamSchema, updateScoreSchema
...
matchRouter.patch('/:id/score', async (req, res) => {
    ...
    try {
        const [existing] = await db
            .select({
                id: matches.id, status: matches.status, startTime: matches.startTime, endTime: matches.endTime,
            })
            .from(matches)
        ...
        const [updated] = await db
            .update(matches)
            .set({
                homeScore: bodyParsed.data.homeScore, awayScore: bodyParsed.data.awayScore,
            })
            .where(eq(matches.id, matchId))
            .returning();
